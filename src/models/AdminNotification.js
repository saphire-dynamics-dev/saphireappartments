...
adminNotificationSchema.statics.createPaymentNotification = async function(transaction) {
  // Get booking request details if available
  let propertyInfo = '';
  if (transaction.metadata && transaction.metadata.propertyTitle) {
    propertyInfo = ` for ${transaction.metadata.propertyTitle}`;
  }

  return await this.createNotification({
    title: 'Payment Received',
    message: `Payment of ₦${(transaction.amount / 100).toLocaleString()} received from ${transaction.customer.firstName} ${transaction.customer.lastName}${propertyInfo}`,
    type: 'payment_received',
    priority: 'high',
    relatedRecords: {
      transaction: transaction._id,
      bookingRequest: transaction.bookingRequest || null
    },
    metadata: {
      ...
      propertyTitle: transaction.metadata?.propertyTitle,
      checkInDate: transaction.metadata?.checkInDate,
      checkOutDate: transaction.metadata?.checkOutDate
    },
