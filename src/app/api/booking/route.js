...
export async function POST(request) {
  try {
    ...
    if (!checkInDate || !checkOutDate || !guests || !firstName || !lastName || !email || !phone) {
      return NextResponse.json({
        success: false,
    ...
    const bookingRequest = new BookingRequest({
      ...
      adminNotes: paymentMethod === 'bank_transfer' ? 'Payment method: Bank Transfer - Awaiting confirmation' : undefined
    });
    ...
    const paymentNote = paymentMethod === 'bank_transfer' 
    ...
    try {
      if (paymentMethod === 'bank_transfer') {
        // For bank transfer, send different email mentioning payment confirmation needed
